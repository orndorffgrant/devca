...
pub(crate) fn create_ca() -> Result<(Vec<u8>, Vec<u8>), String> {
    ...
    let subject_key_id =
        X509Extension::new(None, Some(&ca_cert_context), "subjectKeyIdentifier", "hash")
            .map_err(stringify)?;
    let authority_key_id = X509Extension::new(
        None,
        Some(&ca_cert_context),
...
pub(crate) fn create_cert(
    ...
    let basic_constraints =
        X509Extension::new(None, Some(&ca_cert_context), "basicConstraints", "CA:false")
            .map_err(stringify)?;
    let key_usage = X509Extension::new(
        None,
        Some(&ca_cert_context),
